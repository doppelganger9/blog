...
import { computeMinutesToRead } from '$lib/minutesToRead.js'
import { siteUrl } from '$lib/stores/config.js';

export function createPostsFromMarkdownFilesInDirectory(directory) {
  const slugs = fs.readdirSync(directory)
    .filter(file => path.extname(file) === '.md')
    .map(file => file.slice(0, -3));

  return slugs.map(slug => createPostFromMarkdownFileWithSlug(slug, directory)).sort((a, b) => {
...
function createPostFromMarkdownFileWithSlug(slug, directory) {
