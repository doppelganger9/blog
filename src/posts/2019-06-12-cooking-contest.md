...
You'll see picture of failed cakes, my broken **Orange** car, a talk by **Rich Harris**, leaked API Keys, and some other things for future posts.*
...
I was really fond of its disappearing capability, everything being done at compile time, there was almost nothing left at runtime. Thus, a very lightweight framework at runtime.
...
At the moment of writing, I've spent around 22 cumulated hours on this little webapp, starting from scratch (no prior knowledge).
...
- I used only SvelteV3, starting from [the REPL on the Svelte.dev website](https://svelte.dev/repl/).
- Then I tried [CodeSandbox](https://codesandbox.io/dashboard/recent) which also has a starter for Svelte.
- After using it as a playground, I downloaded the project as a ZIP file, unzipped it, and continued fiddling with it until it started to look like a real app. I frequently commited to git to save my progress.
...
- the last 2 hours were used to rebase interactively my local repo a few times to cleanup a bit, removing sensitive API Keys, adding a license, providing a name and description in package.json.
...
Okay, as you'll see, my designer skills are not the best around. I iterated until it looked fun and *orangey*.
...
> Wow, in this post I showed you my new side-project [cooking-contest.web.app](https://cooking-contest.web.app) which is built with [Svelte](https://svelte.dev). You've seen picture of failed cakes, my broken *Orange* car, a talk by *Rich Harris*, leaked API Keys, and spoilers about upcoming blog posts on using Sapper to rebuild this blog.
>
> I hope you like it, if you have any questions, or just want to say hi or thanks you, [my DMs are open on twitter](https://twitter.com/doppelganger9)!
