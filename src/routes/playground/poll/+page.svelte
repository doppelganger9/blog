<script lang="ts">
  ...
  const functionUrl = '/.netlify/functions/polls'; // 'http://localhost:9999/.netlify/functions/polls' // with netlify dev tools

  registerMoreLabels({
    'fr': {
      'Poll saving to AirTable with Netlify Lambda Function': 'Vote avec Lambda Function liée à AirTable',
  ...
  const ExpectedAnswersEnum = {
    5: "5: Open bar",
    4: "4: certains sujets tabous",
    3: "3: ça dépends",
    2: "2: silencieux",
    1: '1 : pas de vague = "oui oui"',
  } as const;
  type ExpectedAnswers = typeof ExpectedAnswersEnum[keyof typeof ExpectedAnswersEnum];
  const expectedAnswers = Object.values(ExpectedAnswersEnum).sort((a, b) => b.localeCompare(a));
...
<Lang />
<h1>{$i18n`Poll saving to AirTable with Netlify Lambda Function`}</h1>
